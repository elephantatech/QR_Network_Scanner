...
class QRCodeScanner:
    ...
    def start_camera(self):
        ...
        if not self.cap.isOpened():
            raise RuntimeError("Could not open camera. Please check if the application has permission to access the camera (System Settings -> Privacy & Security -> Camera).")
    ...
    def scan_one(self, timeout: float = 30.0, show_window: bool = True) -> Optional[str]:
        """
